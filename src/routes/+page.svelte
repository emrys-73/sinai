    ...
    <div class="w-full md:flex-row flex-col md:gap-6 gap-8 flex justify-between items-center">
        ...
        <!-- SarchBar -->
         <form on:submit={handleSubmit} class="min-w-[45vw] flex flex-row gap-2 w-full">
            ...
            {/if}
            <Input bind:value={$input} class="rounded-full border-opacity-50 text-white placeholder:text-white placeholder:text-opacity-50" disabled placeholder="Smart-Search coming soon!">
                
            </Input>

            <Button type="submit" class="rounded-full text-black" disabled>
              ...
              {#each data.songs as song}
                <Table.Row class="hover:bg-black transition-all duration-300 ease-in-out font-light">
                  ...
                  <Table.Cell class="text-white hidden lg:flex">
                    <a href={song.chords_url} class="hover:text-primary justify-end items-end flex w-full">
                      {#if song.chords_url}
                        <span>
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
